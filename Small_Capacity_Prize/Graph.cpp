...
void shortestPath_to_DestNodes(const Graph & G, int src, std::vector<int> destVertices, std::vector<int>& prev, std::vector<double>& dist){
...
double findTSPApprox(std::vector<int> S_u, std::vector<std::pair<double, double>> verticesCoordinations) {
...
double findTSP2Approx(std::vector<int> S_u, std::vector<std::pair<double, double>> verticesCoordinations) {
    ...
    for (int i = 0; i < S_u.size(); i++)
      for (int j = i + 1; j < S_u.size(); j++){
        ...
        edgesWeights[i][j] = dist;
        edgesWeights[j][i] = dist;
        adj[i].push_back(std::make_pair(edgesWeights[i][j], j));
        adj[j].push_back(std::make_pair(edgesWeights[i][j], i));
      }
...
//    std::cout<<"\n";
    ...
    int n = mstTour.size();
    double lengthOfMstTour = 0;
    for (int i = 0; i < mstTour.size()-1; i++){
//        std::cout<< "("<<mstTour[i]<<", "<<mstTour[i+1]<<", "<<edgesWeights[mstTour[i]][mstTour[i+1]]<<"), ";
        double w = edgesWeights[mstTour[i]][mstTour[i+1]];
        lengthOfMstTour += w;
    }
    lengthOfMstTour += edgesWeights[mstTour[n-1]][mstTour[0]];//shortestPath(G, tour[n-1], tour[0]);
//    std::cout<< "("<<mstTour[n-1]<<", "<<mstTour[0]<<", "<<edgesWeights[mstTour[n-1]][mstTour[0]]<<")\n";
    
    return lengthOfMstTour;  // Return the sum of edge weights of the Minimum Spanning Tree
...
void all_to_all_shortest_paths(const Graph G, std::vector<int> G_Vertices, std::vector<std::vector<int>>& prev_u, std::vector<std::vector<double>>& dist_u) {
       for (auto u:G.getVertices()){
          ...
          shortestPath_to_DestNodes(G, u, G_Vertices, prev, dist);
          prev_u[u] = prev;
          dist_u[u] = dist;  
       }
}
...
void dpForSize(std::vector<int> verticesOfAlg, double capacity, std::vector<std::pair<double, double>> verticesCoordinations, std::vector<double> verticesPrizesOfAlg, std::vector<double> verticesSizesOfAlg, double& totalSize, double& totalPrize, double& totalCost, std::vector<int>& verticesOfAlgRespectingCapacity) {
...
//    std::cout<<"\n";
    ...
    totalCost = findTSPApprox(vecTempVertices, verticesCoordinations);//findTSP2Approx(vecTempVertices, edgesWeightsOfAlg, verticesCoordinations);
...
//            vectTemp.insert(vectTemp.begin(), 0);
      if (cost_x < budget && size_x < capacity) {
        for (auto vrpc : vecRatioPrizeCost) {
            if (cost_x < budget && size_x < capacity) {
               if(size_x + verticesSizes[vrpc.id] <= capacity) {
                    bool flag;
                    ...
                    if (flag) {
                       ...
                       tempCost_x = findTSPApprox(tempVerticesRespectingCapacity, verticesCoordinations);
                       //findTSP2Approx(tempVerticesRespectingCapacity, edgesWeights, verticesCoordinations);
