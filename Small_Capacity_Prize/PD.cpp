...
double prizeTree(const Graph & G, std::list<std::shared_ptr<Edge> > & tree)
{
    int p = 0;
...
// Main function
std::pair<double, double> PD(const Graph &G, double D, std::list<std::shared_ptr<Edge> > & edges, double & upper,
       int & recursions, double & lambda, bool & found, bool recurse)
{
    recursions = 1;
    
    std::cout << " -------- Starting 2ApproxAlg ----------- \n";
    //std::cout << " GRAPH: " << G;
    ...
    if (mst_w <= 0.5*D)
    {
        ...
        upper = G.getPrize();
        //return G.getPrize();
        return std::make_pair(G.getPrize(), mst_w);
    }
    ...
    double currPrize = prizeTree(G,tree);
    ...
    if (recurse){
        ...
        for (auto test_s: altS){
            if (test_s->getPrize() > currPrize){
                ...
                double test_p = prizeTree(G,test_e);
                if ( test_p > currPrize){
                    tree = test_e;
                    currPrize = test_p;
                }
    ...
    //return currPrize;
    return std::make_pair(currPrize, wTree);
}


double PDMod(const Graph &G, double D, std::list<std::shared_ptr<Edge> > & edges,
          double & upper, int & numRuns)
{
    ...
    bool found = false;
    double lambda = 0;
    std::pair<double, double> approx_prize_cost = PD(G, D, edges, upper, recursions,lambda, found, true);
    double prize = approx_prize_cost.first;
    double w = 0;
    for (auto e:edges){
        w += e->getWeight();
    }
    ...
    while (numRuns < 6)
    {
        ...
        std::pair<double, double> approx_prize_cost = PD(G, p, t_edges, t_upper, t_recursions, t_lambda, t_found, true);
        double t_prize = approx_prize_cost.first;
        // Shortcut
